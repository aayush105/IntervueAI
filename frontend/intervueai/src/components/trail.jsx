...
const InterviewPrep = () => {
  ...
  const fetchSessionDataByID = async (showLoader = true) => {
    try {
      ...
      setErrorMsg(error?.response?.data?.message || "An error occurred while fetching session data.")
  ...
  const toggleQuestionPinStatus = async (questionId) => {
    try {
      ...
      setErrorMsg(error?.response?.data?.message || "An error occurred while updating pin status.")
      toast.error(errorMsg)
    }
  }
  ...
  const updateQuestionNote = async (questionId, note) => {
    try {
      ...
      if (response.data && response.data.success) {
        toast.success(note ? "Note saved successfully!" : "Note deleted successfully!")
        await refetchSessionData()
      }
    } catch (error) {
      ...
      <div className="bg-gradient-to-br from-emerald-50 to-teal-50">
        ...
        <div className="container mx-auto pt-4 pb-4 px-4 md:px-0">
          <div className="flex items-center gap-3 mb-6">
            <h2 className="text-2xl font-bold text-gray-900">Interview Q & A</h2>
          ...
          <div className="grid grid-cols-12 gap-4 mt-5 mb-10">
            <div className={`col-span-12 ${openLearnMoreDrawer ? "md:col-span-7" : "md:col-span-8"}`}>
              <AnimatePresence>
                {isLoadingSession
                  ? Array.from({ length: 5 }).map((_, index) => <QuestionCardSkeleton key={index} />)
                        ...
                        >
                          <>
                            <QuestionCard
                              ...
                              answer={data.answer}
                              note={data.note} // Pass note data
                              onLearnMore={() => generateConceptExplanation(data.question)}
                              isPinned={data.isPinned}
                              onTogglePin={() => toggleQuestionPinStatus(data._id)}
                              onUpdateNote={(note) => updateQuestionNote(data._id, note)} // Pass note handler
                            />
